...
$query = $pdo->query("SELECT * from $tabela where igreja_cad = '$id_igreja' or igreja_item = '$id_igreja' order by id desc");
...
if($linhas > 0){
	...
	for($i=0; $i<$linhas; $i++){
		...
		<td>
		<big><a class="btn btn-info btn-sm" href="#" onclick="editar('{$id}','{$codigo}','{$nome}','{$descricao}','{$valor}','{$data_cad}','{$doador}','{$entrada}','{$obs}','{$foto}')" title="Editar Dados"><i class="fa fa-edit "></i></a></big>
		...
		<big><a class="btn btn-primary btn-sm" href="#" onclick="mostrar('{$codigo}','{$nome}','{$nome_ig_item}','{$valor}','{$data_cadF}','{$entrada}','{$obs}','{$foto}')" title="Mostrar Dados"><i class="fa fa-info-circle "></i></a></big>


		<big><a class="btn btn-success btn-sm" href="#" onclick="ativar('{$id}', '{$acao}')" title="{$titulo_link}"><i class="fa {$icone} "></i></a></big>

		<big><a class="btn btn-danger btn-sm {$ocultar_transferir}" href="#" onclick="transferir('{$id}','{$nome}')" title="Transferir Item">	<i class="bi bi-truck"></i> </a></big>

					<big><a style="background: #3fbf7f" class="btn btn-sm {$ocultar_devolucao}" href="#" onclick="devolver('{$id}','{$nome}','{$igreja_cad}')" title="Devolver Item">	<i class="bi bi-truck"></i> </a></big>
...
<script type="text/javascript">
	function editar(id, codigo, nome, descricao, valor, data_cad, doador, entrada, obs, foto){
		$('#mensagem').text('');
	...
	function mostrar(codigo, nome, igreja_item, valor, data_cad, entrada, obs, foto){
