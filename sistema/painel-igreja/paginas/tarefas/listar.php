...
$query = $pdo->query("SELECT * from $tabela where usuario = '$usuario' and igreja = '$id_igreja' and status = 'Agendada' and data <= curDate() and hora < curTime() order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$tarefas_atrasadas = @count($res);

$query = $pdo->query("SELECT * from $tabela where usuario = '$usuario' and status = 'Agendada' and igreja = '$id_igreja' order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_tarefas = @count($res);


$query = $pdo->query("SELECT * from $tabela where usuario = '$usuario' and status = 'Agendada' and data = '$data' and igreja = '$id_igreja' order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$agendadas_hoje = @count($res);

$query = $pdo->query("SELECT * from $tabela where usuario = '$usuario' and data = '$data' and igreja = '$id_igreja' order by hora asc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$linhas = @count($res);
if($linhas > 0){
echo <<<HTML
