...
<td>
	<big><a class="btn btn-primary btn-sm" href="#" onclick="editar('{$id}','{$descricao}','{$valor}','{$fornecedor}','{$vencimento}','{$data_pgto}','{$forma_pgto}','{$frequencia}','{$obs}','{$tumb_arquivo}')" title="Editar Dados"><i class="fa fa-edit "></i></a></big>
...
<big><a class="btn btn-primary btn-sm" href="#" onclick="mostrar('{$descricao}','{$valorF}','{$nome_pessoa}','{$vencimentoF}','{$data_pgtoF}','{$nome_pgto}','{$nome_frequencia}','{$obs}','{$tumb_arquivo}','{$multaF}','{$jurosF}','{$descontoF}','{$taxaF}','{$subtotalF}','{$nome_usu_lanc}','{$nome_usu_pgto}', '{$pago}', '{$arquivo}', '{$telefone_pessoa}', '{$pix_pessoa}', '{$tipo_pessoa}')" title="Mostrar Dados"><i class="fa fa-info-circle"></i></a></big>

<big><a class="{$ocultar} btn btn-sm" style="background:#079934; " href="#" onclick="baixar('{$id}', '{$valor}', '{$descricao}', '{$forma_pgto}', '{$taxa_conta}', '{$valor_multa}', '{$valor_juros}')" title="Baixar Conta"><i class="fa fa-check-square " style="color: #ffff;" ></i></a></big>
...
<script type="text/javascript">
	function editar(id, descricao, valor, fornecedor, vencimento, data_pgto, forma_pgto, frequencia, obs, arquivo){
		$('#mensagem').text('');
