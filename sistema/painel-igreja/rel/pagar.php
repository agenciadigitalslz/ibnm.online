...
if($pago == 'Vencidas'){
	$query = $pdo->query("SELECT * from pagar where vencimento < curDate() and pago != 'Sim' and igreja = '$id_igreja' $sql_user order by id desc");
}else{
	$query = $pdo->query("SELECT * from pagar where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and igreja = '$id_igreja' $sql_user order by id desc");
}
