...
if($filtro_lancamento == 'pastor'){		
	$filtro_lancamentoF = 'Pagamentos Pastores';	
	$filtro_tabela = 'pastores';		
}

if($filtro_lancamento == 'secretario'){		
	$filtro_lancamentoF = 'Pagamentos Secretarios';	
	$filtro_tabela = 'secretarios';		
}
...
$query = $pdo->query("SELECT * from $filtro_tipo where $filtro_data >= '$dataInicial' and $filtro_data <= '$dataFinal' and pago LIKE '%$filtro_pendentes%' and $filtro_lancamento = '$id_pessoa' $sql_igreja order by $filtro_data asc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$linhas = @count($res);
if($linhas > 0){
for($i=0; $i<$linhas; $i++){
...
$igreja_conta = $res[$i]['igreja'];

$query2 = $pdo->query("SELECT * FROM igrejas where id = '$igreja_conta'");
$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
if(@count($res2) > 0){
	$nome_igreja = $res2[0]['nome'];
}else{
	$nome_igreja = '';
}
...
<td style="width:25%">
	...
	<td style="width:20%">R$ <?php echo $nome_igreja ?></td>
...
<td style="width:15%"><?php echo $pix_pessoa ?></td>
    </tr>
