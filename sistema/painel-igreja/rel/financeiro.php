		...
		<table id="cabecalhotabela" style="border-bottom-style: solid; font-size: 8px; margin-bottom:10px; width: 100%; table-layout: fixed;">
			<thead>
				...
				<tr id="cabeca" style="margin-left: 0px; background-color:#CCC">
					...
					<td style="width:15%">LANÇADO POR</td>
				</tr>
			</thead>
		</table>
</div>
...
$query = $pdo->query("SELECT * from $filtro_tipo where $filtro_data >= '$dataInicial' and $filtro_data <= '$dataFinal' and pago LIKE '%$filtro_pendentes%' and referencia LIKE '%$filtro_lancamento%' $sql_igreja order by $filtro_data asc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$linhas = @count($res);
if($linhas > 0){
for($i=0; $i<$linhas; $i++){
...
$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
$nome_usu_lanc = @count($res2) > 0 ? $res2[0]['nome'] : '';
...
<td style="width:15%"><?php echo $nome_usu_lanc ?></td>
