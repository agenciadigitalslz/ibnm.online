...
$filtro_usuario_lanc = isset($filtro_usuario_lanc) ? $filtro_usuario_lanc : '';
if($filtro_usuario_lanc != ''){
	$queryU = $pdo->query("SELECT nome FROM usuarios WHERE id = '".$filtro_usuario_lanc."'");
	$resU = $queryU->fetchAll(PDO::FETCH_ASSOC);
	$nome_lancador_pdf = $resU[0]['nome'] ?? '';
	$texto_lanc = ' • Lançado por: '.mb_strtoupper($nome_lancador_pdf);
}else{
	$texto_lanc = '';
}

$texto_filtro = $texto_tipo_data.' : '.$datas.$texto_lanc;
		...
		<table id="cabecalhotabela" style="border-bottom-style: solid; font-size: 8px; margin-bottom:10px; width: 100%; table-layout: fixed;">
			<thead>
				...
				<tr id="cabeca" style="margin-left: 0px; background-color:#CCC">
					...
					<td style="width:14%">LANÇADO POR</td>
				</tr>
			</thead>
		</table>
</div>
...
$sql_user = ($filtro_usuario_lanc != '' ? " and usuario_lanc = '".$filtro_usuario_lanc."'" : '');
if($pago == 'Vencidas'){
	$query = $pdo->query("SELECT * from receber where vencimento < curDate() and pago != 'Sim' and igreja = '$id_igreja' $sql_user order by id desc");
}else{
	$query = $pdo->query("SELECT * from receber where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and igreja = '$id_igreja' $sql_user order by id desc");
}
...
//PEGAR RESIDUOS DA CONTA
	...
	if($valor_com_residuos > 0){
      ...
      <tr>
<td style="width:22%">
<img style="margin-top: 0px" src="<?php echo $url_sistema ?>img/<?php echo $classe_pago ?>" width="8px">
	<?php echo $descricao ?></td>
<td style="width:10%">R$ <?php echo $valor_finalF ?></td>
<td style="width:17%"><?php echo $nome_membro ?></td>
