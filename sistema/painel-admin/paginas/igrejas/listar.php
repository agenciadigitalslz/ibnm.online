...
if($linhas > 0){
	...
	for($i=0; $i<$linhas; $i++){
		...
		<td><a title="Acessar Igreja" href="../painel-igreja/index.php?igreja={$id}">{$nome}</a> <span class="text-danger">({$prebenda}%)</span></td>
		<td>{$telefone}</td>
		<td>{$data_cadF}</td>
		<td>{$membrosCad}</td>
		<td>{$nome_p}</td>
		...
		<td>
		<a class="btn btn-primary btn-sm" href="#" onclick="editar('{$id}','{$nome}','{$telefone}','{$endereco}','{$foto}','{$pastor}','{$video}','{$email}','{$url}','{$youtube}','{$instagram}','{$facebook}','{$prebenda}')" title="Editar Dados"><i class="fa fa-edit"></i></a>
		...
		<a class="btn btn-primary btn-sm" href="#" onclick="mostrar('{$nome}','{$telefone}','{$email}','{$endereco}','{$data_cadF}','{$matriz}','{$nome_p}','{$prebenda}','{$id}')" title="Mostrar Dados"><i class="fa fa-info-circle"></i></a>


		<a style="background:#22146e; color: #ffff;" class="btn btn-sm" href="#" onclick="arquivo('{$id}', '{$nome}')" title="Inserir / Ver Arquivos"><i class="fa fa-file-o "></i></a>

		<a style="background:#ad2626; color: #ffff;" class="btn btn-sm" href="#" onclick="imagens('{$id}', '{$nome}','{$logo_rel}','{$cab_rel}','{$painel_jpg}','{$carteirinha_rel}')" title="Cadastrar Imagens Relatório"><i class="fa fa-file-image-o"></i></a>
...
<script type="text/javascript">
	function editar(id, nome, telefone, endereco, foto, pastor, video, email, url, youtube, instagram, facebook, prebenda){
	...
	function mostrar(nome, telefone, email, endereco, data_cad, matriz, pastor, prebenda, id){
